...
# Windows: configure Tesseract and Poppler paths
if sys.platform == "win32":
    _TESSERACT = Path(r"C:\Program Files\Tesseract-OCR\tesseract.exe")
    _POPPLER_BIN = None
    # Find Poppler in WinGet packages directory
    _WINGET_PKG = Path(r"C:\Users") / os.environ.get("USERNAME", "hazel") / \
        "AppData/Local/Microsoft/WinGet/Packages"
    for _pkg in _WINGET_PKG.glob("oschwartz10612.Poppler*"):
        _candidate = _pkg / "poppler-25.07.0" / "Library" / "bin"
        if _candidate.exists():
            _POPPLER_BIN = str(_candidate)
            break

try:
    import pytesseract
    if sys.platform == "win32" and _TESSERACT.exists():
...
def pdf_to_image(pdf_path):
    ...
    try:
        kwargs = {"dpi": 400}
        if sys.platform == "win32" and _POPPLER_BIN:
